...
| Future material, oracle data, credentials, network, and outside paths stay outside the worker boundary. | **Exercised for S0-S1.** The recorded Darwin Seatbelt run passed the declared boundary probe for two independent synthetic packs. | The mechanism is Darwin-only, assumes trusted stable code and input roots, excludes malicious same-UID races, and has not admitted the changed private runner. |
...
| Experiment claims remain inside frozen information, scoring, and resource boundaries. | **Partly exercised.** The [synthetic preflight contract](../examples/experiments/preflight/README.md) is closed and mutation-tested, and the runtime-boundary rehearsal checks isolation and recovery. | The full synthetic dress rehearsal is pending: deliberate exhaustion of every admitted budget, baseline execution, scoring, the complete denominator, and a `keep / shrink / stop` record have not run. |
