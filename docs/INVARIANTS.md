...
| Experiment claims remain inside frozen information, scoring, and resource boundaries. | **Partly exercised.** The [synthetic preflight contract](../examples/experiments/preflight/README.md) is closed and mutation-tested; the [G1 evidence-contract twin](../examples/experiments/evidence-contract-twin-v1/README.md) freezes and checks a fictional eight-task denominator, Markdown plus `rg` baseline, budgets, and decision gate without binding a runtime; the runtime-boundary rehearsal checks isolation and recovery; and Issue #24 now has a public [Python corpus-selection and rights freeze](../experiments/corpora/python-annotations-semantics-v1/CORPUS-SELECTION.md), an owner-accepted [source manifest](../experiments/corpora/python-annotations-semantics-v1/SOURCE-MANIFEST.json), and a technically verified byte-identical RST [projection manifest](../experiments/corpora/python-annotations-semantics-v1/PROJECTION-MANIFEST.json). During projection materialization, only the pinned local deterministic builder processed corpus bytes. It emitted no corpus bytes or excerpts to the model, logs, or public artifacts, and no evaluated output exists. | Issue #6 stopped before an admissible private G1 seal or evaluated run. Issue #24 still needs owner acceptance of the exact projection manifest and actual output boundary. Tasks, oracle, budgets, runtime adaptation and rehearsal, SUT and baseline execution, scoring, and a sequential `keep / shrink / stop` decision remain unfrozen and unauthorized. Deliberate exhaustion of every admitted budget has not run. |
