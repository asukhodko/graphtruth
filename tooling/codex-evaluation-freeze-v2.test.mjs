...
} from "./codex-evaluation-freeze-v2.mjs";
import {
  admittedCodexSha256,
...
const ownerAuthorizationRecord =
...
function contractFixture() {
  ...
  return {
    ...
    rubric: {
      ...
      resultClassification: structuredClone(expectedResultClassification),
      severeErrorEffect: "apply-declared-consequence-after-result-classification",
      cellSuccessRule: "only-correct-or-correct-abstention-is-a-successful-cell",
      ...
      medianTimePopulation: "successful-cells-only-after-equal-successful-task-count",
      nonApplicableCounterevidenceRule:
        "satisfied-only-when-oracle-explicitly-marks-not-applicable",
      disagreementProcedure: "preserve-raw-scores-and-resolve-blind-before-arm-reveal",
    ...
    captureTax: structuredClone(expectedCaptureTax),
    roles: {
...
test("a fake two-session run freezes split CORE, audit and success RELEASE", async () => {
  ...
  try {
    ...
    assert.equal(result.ownerAuthorizationRecord, ownerAuthorizationRecord);
...
test("audit rejection is terminal and never creates a success RELEASE", async () => {
  ...
  for (const mutate of [
    (value) => { value.issues[0].code = "unlisted-diagnostic-code"; },
    (value) => { value.issues[0].code = "hypothesis-boundary-invalid"; },
    (value) => { value.issues[0].privatePath = "/private/hidden"; },
    (value) => { value.issues[0].detail = "free text is a hidden channel"; },
  ]) {
    const value = structuredClone(rejected);
    mutate(value);
    assert.throws(
      () => validateAuditResult(value, expected),
      (error) => error.code === "AUDIT_RESULT_INVALID",
    );
  }

  const falseAccept = structuredClone(rejected);
  falseAccept.decision = "accept";
  assert.throws(
    () => validateAuditResult(falseAccept, expected),
    (error) => error.code === "AUDIT_RESULT_INVALID",
  );
});
...
test("a full audit rejection writes TERMINAL and no release directory", async () => {
  ...
  try {
    ...
    assert.equal(result.auditChecklist["identity-and-inputs"], false);
    assert.deepEqual(result.auditIssues, [
      { checkId: "identity-and-inputs", code: "identity-mismatch" },
    ]);
    assert.equal(result.repairPerformed, false);
    assert.equal(result.runPerformed, false);
...
test("CLI arguments are exact and include the pinned rg path", () => {
  const parsed = parseArguments([
    ...
    "--owner-freeze-authorization-record", ownerAuthorizationRecord,
    "--confirm-openai-processing-authorized", "--confirm-no-run-authorized",
  ]);
  assert.equal(parsed.rgPath, "/opt/homebrew/bin/rg");
  assert.equal(parsed.ownerAuthorizationRecord, ownerAuthorizationRecord);
  assert.throws(
    () => parseArguments([]),
  ...
  assert.throws(
    () => parseArguments([
      "--repository", "/repo", "--projection", "/projection", "--private-root", "/private",
      "--codex", "/codex", "--auth-carrier", "/auth", "--rg", "/opt/homebrew/bin/rg",
      "--owner-freeze-authorization-record", evaluationV2PreparationRecord,
      "--confirm-openai-processing-authorized", "--confirm-no-run-authorized",
    ]),
    (error) => error.code === "OWNER_FREEZE_AUTHORIZATION",
  );
});

test("result classes and capture tax are closed structured contracts", () => {
  const mutations = [
    (value) => { delete value.rubric.resultClassification.rules.missing; },
    (value) => { value.rubric.resultClassification.precedence[1] = "contaminated"; },
    (value) => {
      value.rubric.resultClassification.rules.partial.predicate =
        value.rubric.resultClassification.rules.correct.predicate;
    },
    (value) => { delete value.rubric.resultClassification.rules.missing.timeRule; },
    (value) => { delete value.rubric.resultClassification.rules.timeout.timeRule; },
    (value) => { delete value.rubric.resultClassification.rules.interrupted.timeRule; },
    (value) => { delete value.rubric.resultClassification.rules.invalid.timeRule; },
    (value) => { value.rubric.resultClassification.rules.partial.denominatorRule = "drop"; },
    (value) => { value.rubric.resultClassification.rules.partial.taskAggregation = "successful-cell"; },
    (value) => { value.captureTax.activities.pop(); },
    (value) => { delete value.captureTax.interval.startEvent; },
    (value) => { value.captureTax.sharedGraphtruthWorkAllocation.basis = "operator-choice"; },
    (value) => { value.captureTax.endpointBinding.primaryEndpoint = "time-without-capture-tax"; },
    (value) => { value.captureTax.endpointBinding.alternative25PercentThresholdIncludesCaptureTax = false; },
    (value) => { delete value.captureTax.incompleteLog; },
    (value) => { value.captureTax.doubleCounting.maximumAttributionsPerInterval = 2; },
    (value) => { value.captureTax.attribution.futureAutomationCreditAllowed = true; },
  ];
  for (const mutate of mutations) {
    ...
    assert.throws(
      ...
      (error) => ["AUTHOR_RUBRIC_INVALID", "AUTHOR_CAPTURE_TAX_INVALID"].includes(error.code),
    );
  }
});

test("v1 evidence bytes remain exact", async () => {
  const expected = {
    "codex-evaluation-freeze": "6e1df78fd222bf287ad03b3cef236271baf668d37bc51471bf2c87d28cba8b19",
    "codex-evaluation-freeze.mjs": "53f951ddb3ebe82c0d1f3dd6e7fb2dd116e168e20efd9628306912a74fd5a513",
    "codex-evaluation-freeze.test.mjs": "4a5756ea4cf042dc75582eb8a98c60b45ca16c9abb38e6df06bd421aea45ad05",
  };
  for (const [name, digest] of Object.entries(expected)) {
    assert.equal(sha256(await readFile(path.join(toolingDirectory, name))), digest, name);
  }
  assert.equal(
...
test("the public v1 diagnostic receipt is exact, safe, and non-authorizing", async () => {
  const receiptText = await readFile(path.join(
    repositoryRoot,
    "experiments/corpora/python-annotations-semantics-v1/EVALUATION-FREEZE-DIAGNOSTIC.json",
  ), "utf8");
  const receipt = parseStrictJson(receiptText);
  assert.equal(
    sha256(Buffer.from(receiptText, "utf8")),
    "6a22efa5de21bd7f6dcdd62adfd7265a4a07e49c520c4239c57ea9c43cbdd392",
  );
  assert.deepEqual(Object.keys(receipt).sort(), [
    "auditChecklist", "auditDecision", "auditResultSha256", "coreManifestSha256",
    "corpusStatus", "counts", "diagnosticAccess", "documentKind", "evaluationIdentity",
    "experimentId", "futureAuthorization", "issues", "publicDecisionRecord",
    "repairPerformed", "runPerformed", "terminalFileSha256", "terminality", "v1Evidence",
  ]);
  assert.equal(receipt.documentKind, "graphtruth.evaluation-freeze-diagnostic/1");
  assert.equal(receipt.auditDecision, "reject");
  assert.equal(receipt.experimentId, evaluationExperimentId);
  assert.equal(receipt.coreManifestSha256,
    "45ccc83c2514a136d8f69db993a65414ea16ad5731f01aeb908158f89f9def2e");
  assert.equal(receipt.auditResultSha256,
    "872758e111dab28c3cf62245a2c8d222b5a7f17873552ac056a0b6f5a1a1c0cf");
  assert.equal(receipt.terminalFileSha256,
    "410a91aaca18d121a7bafbaf0e117b1f0a4cee04008fb5f717a5fa648705a7bd");
  assert.equal(receipt.repairPerformed, false);
  assert.equal(receipt.runPerformed, false);
  assert.deepEqual(receipt.issues, [
    { checkId: "oracle-and-rubric", code: "result-classes-not-fully-defined" },
    { checkId: "baseline-parity", code: "graphtruth-capture-tax-rule-missing" },
  ]);
  assert.deepEqual(
    Object.entries(receipt.auditChecklist)
      .filter(([, passed]) => !passed)
      .map(([checkId]) => checkId)
      .sort(),
    ["baseline-parity", "oracle-and-rubric"],
  );
  assert.deepEqual(receipt.counts, {
    arms: 2, cells: 64, coreArtifacts: 7, horizons: 4,
    oracleJudgments: 32, severeErrorClasses: 10, tasks: 8,
  });
  assert.equal(receipt.diagnosticAccess.auditResultReads, 1);
  assert.equal(receipt.diagnosticAccess.exactFileOnly, true);
  assert.equal(receipt.diagnosticAccess.extractorSha256,
    "993cf256320e441549c5658bf8ee4b6b6de297aef822e946c2161d645afb6f35");
  assert.equal(receipt.diagnosticAccess.nodeVersion, "v24.4.1");
  for (const field of [
    "adjacentFilesRead", "coreRead", "oracleRead", "privatePathPublished", "tasksRead",
  ]) assert.equal(receipt.diagnosticAccess[field], false, field);
  assert.equal(receipt.corpusStatus.exploratory, true);
  assert.equal(receipt.corpusStatus.confirmatoryUseAllowed, false);
  assert.equal(receipt.corpusStatus.freshConfirmatoryCorpusRequired, true);
  assert.equal(receipt.corpusStatus.freshConfirmatoryIdentityRequired, true);
  assert.equal(receipt.corpusStatus.pythonDisclosureTransferAllowed, false);
  assert.equal(Object.values(receipt.futureAuthorization).every((value) => value === false), true);
  assert.equal(Object.values(receipt.terminality).every((value) => value === false), true);
  assert.deepEqual(receipt.v1Evidence, {
    controllerSha256: "53f951ddb3ebe82c0d1f3dd6e7fb2dd116e168e20efd9628306912a74fd5a513",
    testsSha256: "4a5756ea4cf042dc75582eb8a98c60b45ca16c9abb38e6df06bd421aea45ad05",
    wrapperSha256: "6e1df78fd222bf287ad03b3cef236271baf668d37bc51471bf2c87d28cba8b19",
  });
  assert.equal(receiptText.includes("/Users/"), false);
  assert.equal(receiptText.includes(".nosync"), false);
});
...
test("semantic mutations cannot weaken horizons, budgets, decisions or roles", () => {
  const mutations = [
    ...
    (value) => {
      value.runCard.implementation = "authorized";
    },
    (value) => {
      value.attestations.rehearsalPerformed = true;
    },
    (value) => {
      value.decision.keep.captureTaxIncluded = false;
    },
    (value) => {
      value.baseline.hardTimeoutSecondsPerCell = value.baseline.answerDeadlineSecondsPerCell;
    },
  ];
  for (const mutate of mutations) {
...
test("both prompts repeat source and projection anchors plus exact author grammars", () => {
  ...
  const author = parseStrictJson(buildAuthorPrompt(items, ownerAuthorizationRecord));
  const audit = parseStrictJson(buildAuditPrompt(
    items,
    { "tasks.json": "{}\n" },
    "{}\n",
    ownerAuthorizationRecord,
  ...
  assert.deepEqual(audit.fixedIdentities.auditIssueCodesByChecklist, auditIssueCodesByChecklist);
